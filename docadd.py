...
MIN_CHUNK_SIZE = 300           # Increased from 200 to avoid very small chunks
...
class SemanticChunker:
    ...
    def clean_text(self, text):
        """Clean and normalize text (Unicode-aware, keep Bangla & currency)."""
        # Collapse excessive whitespace but keep newlines for sentence splits
        text = re.sub(r'[^\S\n]+', ' ', text)
        # Keep Unicode letters/numbers incl. Bangla, common punctuation, currency
        text = re.sub(
            r"[^0-9A-Za-z\u0980-\u09FF\s\.\!\?\;\:\,\-\(\)\[\]\"\'€£৳₹%/–—|•…]+",
            " ",
            text
    ...
    def split_into_sentences(self, text):
        """Split text into sentences (Bangla-aware with NLTK fallback)."""
        cleaned_text = self.clean_text(text)
        # If Bangla characters present, use Bangla-friendly split on '।' and end punctuations
        if re.search(r'[\u0980-\u09FF]', cleaned_text):
            # Normalize spacing around Dari
            ...
            try:
                ...
                # Fallback regex-based sentence split
                sentences = re.split(r'(?<=[\.\?\!])\s+', cleaned_text)
...
try:
    ...
    print(f"[ERROR] Failed to initialize sentence embedding model: {e}")
    raise
...
for pdf_path in PDF_PATHS:
    ...
    if not raw_text.strip():
        print(f"[WARNING] Empty text from: {pdf_path}")
        continue
